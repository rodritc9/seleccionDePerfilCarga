...
PerfilUPN::PerfilUPN(const PerfilUPN& orig) {
}

PerfilUPN::PerfilUPN(vector<string> p_name, vector<string> valores){
	for(size_t i=0; i<p_name.size(); i++){
		if(p_name[i]=="h" || p_name[i]=="h "){
			this->h=atoi(valores[i].c_str());
			this->id = "UPN " + valores[i];
			//cout << "__" << i << endl;
...
ostream &operator <<(ostream& os, const PerfilUPN &perfil){
